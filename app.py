...
def init_db():
    with db() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS jobs(
            id TEXT PRIMARY KEY, created_at TEXT, nome TEXT, arquivos TEXT,
            total INTEGER, enviar INTEGER, arriscado INTEGER, invalido INTEGER, smtp_used INTEGER)""")
init_db()
...
<div class=card>
  ...
  {% if jobs %}
  <table><tr><th>Data</th><th>Arquivos</th><th>Total</th><th>Enviar</th><th>Arriscado</th><th>Invalido</th><th></th></tr>
  {% for j in jobs %}
  <tr>
    <td>{{ j['created_at'] }}</td><td class=muted>{{ j['arquivos'] }}</td>
    <td>{{ j['total'] }}</td>
    ...
    <td><a href="{{ url_for('job', job_id=j['id']) }}">abrir</a></td>
  </tr>
  {% endfor %}</table>
...
<div class=card>
  ...
  <p class=muted>Arquivos: {{ j['arquivos'] }} {% if j['smtp_used'] %}&middot; SMTP: ativo{% else %}&middot; SMTP: nao usado{% endif %}</p>
  <div class=kpi>
  ...
  <p class=muted>Dica: mande os de "enviar". "Arriscado" = role/catch-all/nao verificado (devagar). "Invalido" = nao envie.</p>
</div>"""
...
def upload():
    ...
    recs, smtp_used = validator.validate(emails, smtp=smtp)
    ...
    with db() as c:
        c.execute("INSERT INTO jobs VALUES(?,?,?,?,?,?,?,?,?)", (
            ...
            "", ", ".join(names)[:300], len(recs),
            len(buckets["enviar"]), len(buckets["arriscado"]), len(buckets["invalido"]), int(smtp_used)))
